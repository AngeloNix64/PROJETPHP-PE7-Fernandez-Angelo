...
if (isset($_POST['terme'])) {
    ...
    // Requête SQL pour rechercher dans la base de données (table "anime")
    $sql = "SELECT id, name, YEAR(released_date) AS release_year, cover_image_anime
            FROM anime
        ...
        if ($stmt->rowCount() > 0) {
            ...
            foreach ($chunkedResults as $resultGroup) {
                ...
                foreach ($resultGroup as $row) {
                    include 'result_template_anime.php'; // Inclure le modèle pour chaque résultat
                }
                echo '</div>';
            }
        } else {
